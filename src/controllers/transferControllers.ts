        ...
        if (
          ...
          if (sender_AccountBalance > amount) {
            ...
            if (sucessful_transfer) {
              const beneficiary_old_Account_Balance =
                validated_Beneficiary.accountBalance;
              const beneficiary_new_AccountBalance =
                amount + beneficiary_old_Account_Balance;
              ...
              const expected_beneficiary_AccountBalance =
                expected_beneficiary_balance.accountBalance;

              if (
                beneficiary_new_AccountBalance !==
  ...
  try {
    ...
    if (decodedToken.email) {
      ...
      if (amount < user_accountBalance) {
        ...
        return res.status(400).json({
          message:
            "You do not have sufficient balance to execute this savings transfer",
        });
      }
    }
  } catch (error) {
  ...
  try {
    ...
    if (decodedToken.email) {
      ...
      if (user_account_balance > amount) {
        ...
        const company_details: any = await Company.findOne({
